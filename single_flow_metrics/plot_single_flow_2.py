...
def plot_matrix_C(df):
    # FIX 1: Use .str.contains() to catch all "Bandwidth_Scaling_..." rows
    ...
    # FIX 2: Update title. We removed the "Fixed Buffer" variable, 
    # so we are no longer comparing Buffer Sizes here. We are comparing Algorithms.
    plot_grid(subset, "bw_mbps", "Bandwidth (Mbps)", 
...
def plot_matrix_D(df):
    # 1. Filter the data
    subset = df[df["matrix"].str.contains("BufferSize")].copy() # Use .copy() to avoid SettingWithCopy warnings
    
    if subset.empty: 
        return

    # 2. FIX: Force the X-axis column to be float (numbers) instead of strings/objects
    # This ensures the plotter knows 0.1 < 1.0 < 10.0 and connects them
    subset["bdp_mult"] = subset["bdp_mult"].astype(float)
    
    # 3. FIX: Sort the data by the X-axis
    # Matplotlib sometimes refuses to draw lines if data is out of order (e.g. 1.0, 0.1, 5.0)
